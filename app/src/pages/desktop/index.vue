...
import DesktopShell, { type DesktopModule } from '../../components/DesktopShell.vue';
...
const activePanel = computed<PanelRef | null>(() => {
  const map: Partial<Record<DesktopModule, PanelRef | null>> = {
...
function handleSearch(value: string) {
  ...
  activePanel.value?.setSearch?.(value);
}

function handlePrimary() {
  activePanel.value?.openCreate?.();
}

async function handleSecondary() {
  if (active.value === 'brands' || active.value === 'companies') {
    active.value = 'import';
    searchValue.value = '';
    await nextTick();
...
<template>
  <DesktopShell
    ...
    :search-placeholder="currentMeta.search"
    :search-value="searchValue"
    :primary-visible="!!currentMeta.primary"
    :secondary-visible="!!currentMeta.secondary"
    :primary-text="currentMeta.primary || '新增'"
    :secondary-text="currentMeta.secondary || '导入数据'"
    @switch="switchModule"
    @search="handleSearch"
  ...
  >
    <DashboardPanel
      ...
      ref="dashboardRef"
      @open-lead="openLead"
      @open-module="switchModule"
    />
    <LeadPanel
      v-else-if="active === 'my-leads'"
      ref="myLeadRef"
      scope="mine"
    />
    <LeadPanel
      v-else-if="active === 'all-leads'"
      ref="allLeadRef"
      scope="all"
    />
    <BrandPanel
      v-else-if="active === 'brands'"
      ref="brandRef"
      @request-import="switchModule('import')"
      @open-lead="openLead"
    />
    <CompanyPanel
      v-else-if="active === 'companies'"
      ref="companyRef"
      @request-import="switchModule('import')"
      @open-lead="openLead"
    />
    <TypePanel
      v-else-if="active === 'types'"
      ref="typeRef"
    />
    <ImportPanel
