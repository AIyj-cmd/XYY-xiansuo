...
def load_daemon_config(path: str | Path) -> TransportConfig:
    """Dynamic capture mode intentionally has no static peer allowlist.

    The old ``load_config`` remains the only path for one-shot static capture.
    """
    config_path = Path(path)
    try:
        config_path = require_private_file(config_path, kind="daemon 配置文件")
        raw = json.loads(config_path.read_text(encoding="utf-8"))
    except (OSError, SecurityError, json.JSONDecodeError) as exc:
        raise ConfigError("daemon 配置文件必须是当前用户拥有的 0600 JSON 普通文件") from exc
    if not isinstance(raw, dict) or set(raw) != {"account_id", "ilink_token", "hmac_key"}:
        raise ConfigError("daemon 配置字段必须且只能包含 account_id、ilink_token、hmac_key")
    account_id = _id(raw.get("account_id"), "account_id"); token = raw.get("ilink_token")
    if not isinstance(token, str) or not token or len(token) > 8192: raise ConfigError("配置字段 ilink_token 无效")
    encoded_key = raw.get("hmac_key")
    if not isinstance(encoded_key, str): raise ConfigError("配置字段 hmac_key 无效")
    try: key = base64.b64decode(encoded_key, validate=True)
    except Exception as exc: raise ConfigError("配置字段 hmac_key 必须为 base64") from exc
    if len(key) < 32: raise ConfigError("hmac_key 解码后至少需要 32 字节")
    return TransportConfig(account_id, token, (), key)
