...
> 状态：已由用户确认，作为阶段五路线关闭与企业微信外部门禁核验的批准设计边界
> 审计日期：2026-07-31
...
4. 公众号/服务号属于官方普通微信接收路径候选，但要求关注、OpenID/授权，
   ...
   当日规则重新核验。
5. 若企业微信在未来通过主体、自建应用和员工 UserID 外部门禁并获准实施，真实
   渠道前仍须重新确认迁移 `007`、投递模型和兼容方案；**当前不得创建迁移
   `007`**，不得修改迁移 `001` 至 `006`。
...
- 正式生产首选：企业微信自建应用消息；
- 普通微信：OpenClaw daemon 与 Direct iLink 均 No-Go，阶段五A关闭；
- 企业微信群 Webhook：仅作为未来管理群通知或应急补充，不承担员工个人定向
  通知；
- Hook、桌面 RPA、WeChatFerry、wxauto、Gewechat、旧版 Wechaty 普通微信
  Puppet 和未经授权的逆向协议：不采用。

阶段五A iLink 隔离 PoC 已关闭。当前后续工作仅从企业微信主体、自建应用和员工
UserID 的外部门禁核验开始；在这些门禁核验并另获实施批准前，不创建迁移 `007`，
不写企业微信代码，也不改造通知 Worker。
...
| 企业微信自建应用 | 正式生产首选 | 仅核验企业微信主体、自建应用和员工 UserID 外部门禁；尚不实施 |
...
| `server/src/db.ts` | 迁移 `004` 创建通知表，`005/006` 创建并扩展 AI 日志 | `001–006` 冻结；未来真实渠道若获重新批准，才重新确认新迁移 `007` 方案 |
...
结构只是在企业微信外部门禁通过、设计重新确认并获实施批准后的参考：
...
- 方案 A（全 Ubuntu）：企业微信和公众号等官方 HTTP API 的未来部署候选。
...
## 7. 迁移 `007` 与投递数据模型（未来企业微信实施参考，非当前任务）

### 7.1 结论

若企业微信外部门禁通过且重新批准真实渠道实施，届时需要重新确认迁移 `007`、
`notification_deliveries` 与 `notification_channel_bindings` 的完整设计。本轮
**不得创建迁移 `007`**，本章不构成当前实施授权。
...
当前不写、不运行渠道实现测试。未来企业微信外部门禁通过且另获实施批准后，测试
至少应覆盖企业微信主体/自建应用/员工 UserID 核验、凭证隔离、接收人绑定、幂等、
`result_unknown`、权限与 H5 链接、Fake Gateway、Mock 回归、迁移兼容和回滚。
iLink 的历史测试矩阵仅可在其官方能力解锁后的重新审计中作为参考。
...
当前唯一后续顺序为：核验企业微信主体 → 核验可创建自建应用 → 核验员工
UserID 与应用可见范围 → 向主代理报告外部门禁结果。门禁通过前不得创建迁移
`007`、不得写企业微信代码、不得运行 Gateway 或真实渠道投递。
...
| P1 | 企业微信主体、自建应用、员工 UserID 与可见范围未核验 | 当前仅执行外部门禁核验，不实施 |
| P1 | 真实渠道的迁移、投递模型和兼容策略尚未重新批准 | 不创建迁移 `007`，不改造 Worker |
| P2 | 凭证、隐私、结果未知与公网暴露 | 未来设计必须保留隔离、最小数据、fail-closed 和人工确认边界 |

Hook、RPA、逆向协议、Windows 客户端自动化均不作为替代路线；企业微信门禁未满足
时，系统维持站内通知和 Mock 验证。
...
- 当前不得创建迁移 `007`、不得写企业微信代码；
- 当前仅从企业微信主体、自建应用和员工 UserID 的外部门禁核验开始。

企业微信外部门禁为：公司主体可用、可创建自建应用、员工 UserID 可取得且应用
可见范围可核验。未满足时不得进入实施；满足后仍须重新确认设计并取得实施批准。
...
8. **企业微信应作为主渠道还是备用？** 正式生产首选。
9. **当前可否创建迁移007？** 不可。
10. **当前可否写企业微信代码？** 不可。
11. **当前唯一可继续的工作是什么？** 核验企业微信主体、自建应用和员工 UserID 外部门禁。
...
18. **未来企业微信是否仍需迁移/投递设计？** 是，但需门禁通过后重新确认，当前不实施。
19. **未来 Gateway 是否需隔离凭证？** 是，作为未来实施设计边界。
20. **如何处理发送结果未知？** 未来须 fail closed 并人工确认；当前无真实发送。
21. **是否允许接收微信业务指令？** 不允许。
22. **是否允许真实客户数据进入当前阶段？** 不允许。
23. **企业微信门禁不通过怎么办？** 继续使用站内通知和 Mock，不以 iLink 或逆向方案替代。
24. **当前是否存在可执行实现任务？** 不存在，只有只读外部门禁核验。
25. **推荐后续顺序？** 完成企业微信外部门禁核验 → 报告结果 → 重新确认实施设计与授权。

## 21. 当前任务拆分

无。阶段五A已关闭；企业微信尚处于外部门禁核验阶段，当前不创建迁移 `007`、
不写企业微信代码、不启动渠道服务。
