...
> 状态：已由用户确认，作为阶段五渠道PoC与正式渠道后续工作的批准设计边界
> 审计日期：2026-07-31
...
1. **个人普通微信自动化目前不批准作为正式生产渠道。**
   ...
   成功，也不能证明长期生产可用。
2. **腾讯官方 `openclaw-weixin` / iLink 能力是普通微信方向的首选 PoC
   候选，但暂不是生产结论。**其官方发送协议要求 `to_user_id` 和
   `context_token`。必须先证明定时任务在没有新鲜入站会话的情况下能否合法、
   稳定地主动发送。
3. **企业微信自建应用消息是当前正式生产路径中风险最低的选择。**
   它使用官方 API 和稳定企业成员身份，但接收端是企业微信，不能冒充个人微信
   自动化。
4. 公众号/服务号属于官方普通微信接收路径候选，但要求关注、OpenID/授权，
   且主动消息类型、资格、频率和时效受官方规则限制；接入前必须按账号主体和
   当日规则重新核验。
5. 真正接入任何真实渠道前，必须追加迁移 `007`，将业务通知
   `notification_logs` 与渠道投递 `notification_deliveries` 分离，并建立
   `notification_channel_bindings`。不得修改迁移 `001` 至 `006`。
6. 需要独立 WeChat Gateway。Gateway 不访问业务数据库、不调用 DeepSeek、
   不处理微信指令，只接收已经校验的最小消息。
7. V1 只发送文本和需登录的 H5 链接，完全禁止接收微信命令。

冻结后的渠道方向：

- 正式生产首选：企业微信自建应用消息；
- 普通微信：腾讯官方 iLink/ClawBot 只做完全隔离的能力 PoC；
- 企业微信群 Webhook：仅作为未来管理群通知或应急补充，不承担员工个人定向
  通知；
- Hook、桌面 RPA、WeChatFerry、wxauto、Gewechat、旧版 Wechaty 普通微信
  Puppet 和未经授权的逆向协议：不采用。

阶段五拆分为彼此独立的阶段五A iLink隔离PoC和阶段五B企业微信正式渠道。
iLink PoC 不连接业务数据库、真实 `notification_logs` 或 DeepSeek，也不修改
现有通知 Worker。企业微信主体和成员条件是阶段五B实施前的外部门禁，不影响
本文冻结。
...
| 位置 | 当前行为 | 阶段五影响 |
|---|---|---|
| `server/src/notification-worker.ts` | 直接构造 `MockNotificationChannel`，领取统一队列并发送 | 必须改为渠道路由与 delivery 领取；不能把微信 SDK 直接塞入 Worker |
| `server/src/services/mock-notification-channel.ts` | 单 Mock 渠道，确定性 receipt | 继续保留，作为回归、演练和回滚渠道 |
| `server/src/services/notification.ts` | `notification_logs` 内完成租约、重试、TTL 和发送终态 | 业务事件状态与渠道 delivery 状态需要拆分 |
| `server/src/notifications/snapshot.ts` | 严格解析事件快照并转换 `NotificationMessage` | 可复用；真实渠道只能接收转换后的消息 |
...
| `server/src/db.ts` | 迁移 `004` 创建通知表，`005/006` 创建并扩展 AI 日志 | `001–006` 冻结；真实渠道通过新迁移 `007` 扩展 |
...
这正是阶段三“单 Mock 渠道”批准的简化模型。阶段三设计也明确要求真实渠道
或多渠道 fallback 接入前重新评估投递明细。继续把普通微信、企业微信和 Mock
的状态都写进同一行，会导致渠道切换覆盖历史、无法表达 `result_unknown`、
难以防止双发，因此阶段五不能沿用该简化。
...
结论：**官方候选，可作为普通微信接收的备用路线；实现前需以实际账号权限做
资格核验，不作为本轮默认主方案。**
...
结论：**冻结为阶段五正式生产通知渠道的首选。**用于内部员工定向通知，使用
稳定企业成员 UserID 和官方 API，通过 Ubuntu Channel Gateway 发送。公司企业
微信主体、自建应用和成员身份是否具备，是正式实施前的外部门禁；本轮不创建
应用、不接入 API。

企业微信群 Webhook 只保留为未来管理群通知或应急补充候选，不作为员工个人
定向通知的主要实现。
...
是后端定时主动产生，并不保证接收人刚刚给机器人发过消息。PoC 必须确认：
...
的报告。这些问题不是官方服务级承诺，却足以要求 PoC 覆盖：
...
结论：**普通微信目标渠道的首选隔离 PoC，但在主动发送语义获得官方证据并经
受控验证前，不批准生产。**
...
| Tencent iLink/ClawBot | 官方机器人协议 | 腾讯官方仓库 | 新能力，需持续核验 | HTTP Gateway | 预计是 | 未证明需要 | 二维码/机器人 token；接收人和上下文 token | **待 PoC 证明定时主动发送** | 有接口响应，exactly-once 未证明 |
...
|---|---|---|---|---|---|---|---|
| 企业微信自建应用 | 低，仍受官方规则约束 | Secret 需隔离和轮换 | 低至中 | 官方政策为准 | 高 | 高 | **正式生产首选** |
| 公众号/服务号 | 中，受账号资格和主动发送规则约束 | Token/OpenID 需保护 | 中 | 官方政策为准 | 条件性 | 中 | **官方备用候选** |
| Tencent iLink/ClawBot | 未知到中；新能力和上下文限制未证实 | bot token/会话需 Gateway 隔离 | 中 | 插件 MIT；服务条款和费用仍需核验 | 未批准 | 高 | **仅推荐隔离 PoC** |
...
|---|---|---|---|---|---|---|
| 企业微信自建应用 | 企业成员 `userid`，需应用可见且启用 | 官方应用消息；文本当前文档上限为 2048 字节，最终以实施日规则为准 | HTTP健康、token刷新；无需桌面会话 | 企业管理员和成员体系处理 | 企业应用凭证，仅放渠道 Adapter/Gateway | 可长期运维 |
| 公众号/服务号 | OpenID + 关注/授权 | 客服、模板或订阅消息；受窗口、资格、次数和内容规范限制 | 官方API健康；Access Token受控刷新 | 用户关注/授权 | Access Token，不保存用户微信密码 | 条件性可运维 |
| Tencent iLink/ClawBot | `to_user_id` + 会话 `context_token` | 支持文本及部分媒体；本项目V1仍只发文本；主动定时能力待证 | 需心跳、同步游标、会话超时和人工重登；自动重连不能绕过验证 | 二维码、手机确认，异常时人工处理 | bot token、context token和游标只在Gateway | 仅PoC |
...
独立渠道安全域
├── 固定合成消息 PoC Runner
└── xiansuo-wechat-gateway
        └── 官方 iLink/ClawBot（PoC）
```

PoC Runner 不读取业务数据库、DeepSeek 或真实 outbox。只有 iLink 以后重新通过
生产审批，才允许设计 `notification-worker → Gateway` 的普通微信真实投递。

企业微信/公众号官方 API 也通过独立渠道 Gateway：
...
只持有调用 Gateway 的最小认证密钥，不能读取企业微信 Secret、公众号 Token
或 iLink 会话。这样官方渠道和会话型渠道遵循同一凭证隔离边界。

### 6.2 Ubuntu 与 Windows

- 方案 A（全 Ubuntu）：对官方 HTTP API 和可在 Linux 运行的官方
  iLink Gateway 可行，是 PoC 首选。
- 方案 B（Ubuntu + Windows）：只有 Hook/RPA/PC 客户端方案才需要；由于这些
  方案不推荐，**当前不建设 Windows 节点**。
- 方案 C（官方 API）：企业微信和符合资格的公众号最适合正式生产。
...
**需要迁移 `007`，同时创建 `notification_deliveries` 和
...
- 只为专用测试接收账号手工配置 `to_user_id` 和测试所需上下文；
- 配置只存在于隔离 PoC Gateway，不写业务数据库和 binding 表；
- 不建立业务员工绑定，不接真实 outbox；
- PoC 期间收到的内容只用于验证会话和 `context_token`，不进入业务命令路由，
  不查询或修改任何业务数据。

如果 iLink 以后重新获批接入真实 outbox，必须重新设计并审批正式挑战验证和
binding assertion 流程；当前设计不预先批准该生产能力。
...
正式生产 Gateway 只允许 notification-worker 调用；阶段五A隔离 PoC Gateway
只允许本地合成消息 PoC Runner 调用，两者不得共用实例、凭证或队列。Fastify、
H5、AI Scheduler 和公网客户端没有网络权限。若跨节点，优先 WireGuard/VPN +
mTLS；不得为了方便将登录二维码或发送端点公开到公网。
...
iLink 当前不是正式渠道，也不是企业微信的自动 fallback。V1 不存在从 iLink
自动切换到企业微信的投递链路。

站内通知保持现状，它是用户登录后的信息入口，但不能伪造成外部渠道 delivery。
企业微信明确失败时，任务保留为 failed 并可诊断，站内通知继续存在。
...
```text
阶段五A：腾讯官方 iLink 隔离 PoC
阶段五B：企业微信自建应用正式渠道
```

阶段五A只验证普通微信官方渠道能力。它不开发迁移 `007`，不修改生产 Worker，
不连接业务数据库、真实 `notification_logs` 或 DeepSeek。

阶段五B只有在用户后续确认企业微信主体、自建应用和成员身份条件具备后，才进入
正式开发。企业微信主体可用性是生产实施外部门禁，不阻塞本文冻结或阶段五A。
如果公司暂时不能提供企业微信主体：

- 不得把 iLink 自动升级为正式渠道；
- 不得改用 Hook、RPA 或逆向机器人；
- 所有正式真实渠道继续关闭；
- 系统继续使用站内通知和 Mock 验证。

### 15.2 推荐 PoC 候选

只对腾讯官方 iLink/ClawBot 做隔离 PoC。不得用 Hook、RPA、WeChatFerry、
wxauto、Gewechat、旧版普通微信 Puppet 或逆向客户端替代。
...
功能 PoC 持续 3 至 7 天，至少验证：

1. 官方资格和条款；
2. 二维码、设备确认和二次验证；
3. `to_user_id`、`context_token` 的来源和有效期；
4. 从未与 Bot 对话的测试用户能否接收；
5. 刚与 Bot 对话后能否接收；
6. 超过 24 小时或上下文失效后能否接收；
7. 是否要求用户持续互动；
8. 单条文本、长度限制和字符兼容；
9. 重复请求、幂等和 Gateway 重启；
10. 会话持久化、主机重启和退出；
11. 登录失效是否必须重新扫码；
12. 成功响应、响应丢失和结果分类；
13. 网络断开、限流、接收人不存在和账号限制；
14. 会话清理、错误分类和 secret 不落日志；
15. 是否产生不可接受的持续人工维护成本。

如果 iLink 不能在无需用户近期互动的情况下可靠发送定时通知，则冻结结论为：

```text
不满足XYY-xiansuo主动提醒场景
```

不得使用非官方协议、Hook 或 RPA 绕过。

### 15.5 稳定性观察：30天

只有功能 PoC 通过后，才允许进入 30 天稳定性观察。观察期间仍然：
...
**功能 PoC 和 30 天观察成功均不等于批准生产。**任何真实 outbox 接入仍需重新
进行生产渠道审批。

如果官方 iLink 不能在没有有效入站上下文时主动发送，普通微信定时通知方案即
被阻塞；正式渠道继续采用企业微信自建应用。
...
- 专用测试账号和合成消息；
- 从未互动、刚互动和超过24小时三个会话场景；
- context token取得、过期和失效；
- 登录、扫码、设备确认、Gateway/主机重启；
- 重复请求、网络断开、响应丢失和结果未知；
- 不读取业务数据库、真实outbox或DeepSeek；
- 不修改通知Worker，不发送客户数据；
- 3至7天功能记录和可选30天观察指标完整。
...
- 历史 Mock 不被改写；
- 业务通知与 delivery 状态分离；
- legacy/v2 Worker切换、双进程互斥和队列预检；
- 父状态聚合的全部优先级；
- 规则关闭取消 blocked/pending/retry_wait，并正确处理 sending/unknown；
- 失败整体回滚、完整性和外键检查。

### 16.3 绑定

- 企业微信UserID存在、启用和应用可见范围核验；
- iLink PoC测试接收ID手工配置且不进入业务绑定表；
- 重复绑定；
- 一个外部 ID 绑定多个用户被拒绝；
- 一个用户绑定多个已批准渠道；
- 停用用户、失效绑定、接收人不存在；
...
- 合法/非法签名；
- 过期时间戳、重复 nonce、请求体篡改；
- 超大请求、非 allowlist 来源；
- mTLS 失败；
- secret 轮换；
- Gateway 无业务 DB 权限；
- 请求不含 Prompt、JWT 或业务对象。

### 16.5 发送与幂等

- 成功、重复请求、确定性 receipt；
- 响应丢失和 `result_unknown`；
- 超时、离线、登录过期、账号受限；
- 接收人不存在、消息过长、限流；
- Gateway/客户端重启；
- Worker 租约恢复；
- 发送成功后其他 delivery 取消；
- 未知结果不自动重试或 fallback。
- 绑定版本或 external ID hash 变化后取消为 `binding_stale`；
- iLink 本地 receipt 不能单独形成 sent。

### 16.6 降级

- 企业微信明确失败时保留站内通知和失败审计；
- 企业微信无绑定/绑定失效；
- result_unknown不自动fallback；
- V1不把iLink作为fallback；
- 未来多渠道能力保持默认关闭；
- v2 父重试接口拒绝，delivery重试状态和乐观锁正确。
...
- URL 无 JWT；
- Gateway 日志无客户全文；
- 渠道凭证不进入 API、Scheduler、Worker 业务日志或 SQLite；
- 消息长度和条目上限；
- 无权限 H5 链接访问被拒绝。

### 16.8 回归

- 原 121 项后端测试及新增测试全部通过；
- H5 构建通过；
- 不执行小程序构建；
- `owner_changed`、AI Scheduler、Mock 和通知 Worker 回归；
- Fake Gateway 测试不调用真实微信；
- `server/data` 不变化。
...
1. 修订并冻结阶段五设计；
2. 提交设计基线；
3. 开发完全隔离的 iLink PoC Gateway；
4. 使用固定合成消息完成 3 至 7 天功能 PoC；
5. 根据结果决定是否进入 30 天稳定性观察；
6. 单独审计企业微信主体、自建应用和成员绑定条件；
7. 设计并实现迁移 `007`；
8. 实现 `notification_deliveries` 和绑定服务；
9. 实现通用 Gateway 安全协议；
10. 实现企业微信自建应用 Adapter；
11. 使用 Fake Gateway 独立测试；
12. 使用单个内部测试用户灰度企业微信；
13. 完成 result_unknown、人工确认和回滚验收；
14. iLink 只有重新批准后才能接入真实 outbox。

阶段五A PoC 不得提前创建迁移 `007`、修改生产 Worker 或连接真实 outbox。

紧急停止：

1. 关闭渠道规则；
2. 关闭 `WECHAT_CHANNEL_ENABLED`；
3. 停止 Gateway；
4. 停止真实渠道 delivery 领取；
5. 保留 pending、failed 和 `result_unknown` 审计；
6. 撤销/轮换会话和 Gateway secret；
7. 恢复 Mock，不回放旧真实 delivery。

回滚不执行破坏性 down migration，不删除 delivery、绑定或历史 Mock。代码回滚
前将真实渠道 delivery 隔离，防止旧 Worker 把它误当成历史单渠道任务。

## 18. 风险清单与生产阻塞项

| 级别 | 风险 | 当前处置 |
|---|---|---|
| P1 | iLink 主动通知是否需要有效入站 `context_token` 未证明 | 阻止普通微信生产，先 PoC |
| P1 | 平台授权、商业使用和账号资格未确认 | 实现前以官方条款书面核验 |
| P1 | 普通微信自动化结果未知可能重复发送 | `result_unknown` 隔离，禁止自动 fallback |
| P1 | 未拆 delivery 会覆盖多渠道历史 | 迁移 `007` 是真实渠道前置门禁 |
| P1 | 接收人按姓名匹配会误发 | 企业微信稳定UserID人工核验；PoC测试ID手工配置 |
| P2 | 会话凭证或二维码泄露 | 独立 Gateway、安全域、0600、加密和轮换 |
| P2 | Gateway 公网暴露被伪造调用 | 私网/VPN、mTLS、HMAC、nonce、防重放 |
| P2 | 客户隐私经微信泄露 | 最小文本、脱敏、需登录 H5 链接 |
| P2 | 渠道掉线和协议升级 | 健康状态、停止领取、人工恢复 |
| P2 | fallback 双发 | 只对明确失败降级，首个 sent 取消其他 |
| P3 | 无官方 message ID 时审计有限 | 保存本地 receipt，明确不能证明 exactly-once |

阻止正式生产的条件包括：

- iLink 主动发送语义不明确；
- 无明确官方使用授权或账号资格；
- 没有迁移 `007` 和 delivery 状态机；
- 企业微信没有经过核验的稳定 UserID 绑定；
- Gateway 需要公网裸露；
- 不能隔离会话凭证；
- 不能识别发送结果未知；
- 需要 Hook、破解、绕过验证或反检测；
...
- 企业微信自建应用为正式生产首选；
- 不建设 Windows 节点；
- 所有真实渠道统一经过独立 Gateway；
- 正式渠道需要迁移 `007`、delivery 和 binding 两张表；
- 默认不发送任何客户名称；
- iLink 功能 PoC 为 3 至 7 天；
- 普通微信成为生产候选前需 30 天稳定性观察；
- PoC 失败不得转用 Hook、RPA 或逆向协议；
- V1 禁止接收微信业务指令；
- `result_unknown` 不自动重试或 fallback。

以下是对应 PoC 或企业微信实施前的外部门禁，不阻塞本文设计冻结：

- 公司是否已有企业微信主体；
- 是否可以创建自建应用；
- 是否能取得员工企业微信 UserID；
- 专用 iLink 测试 Bot/账号和测试接收账号是否准备完成。

当前没有阻塞设计冻结的产品未决项。外部门禁未满足时不得进入对应实施；企业
微信条件暂不具备时，正式真实渠道继续关闭，系统维持站内通知和 Mock 验证。

## 20. 25 个必须回答的问题

1. **普通微信是否适合作为正式生产渠道？** 当前不适合；官方 iLink 只批准
   PoC，Hook/RPA/逆向方案不批准。
2. **推荐的普通微信实现方案？** 腾讯官方 iLink/ClawBot + 独立 Gateway，
   前提是 PoC 证明主动发送和授权。
3. **是否只能先做隔离 PoC？** 是。
4. **企业微信应作为主渠道还是备用？** 冻结为正式生产首选。
5. **Ubuntu能否独立承载？** 官方 API 和官方 iLink Gateway 预计可以；PoC
   需证明。Hook/RPA 不可以。
6. **是否需要Windows节点？** 当前不需要，也不建议建设。
7. **是否需要独立Gateway？** 普通微信会话型方案必须需要；官方 API 也保持
   独立 Adapter 边界。
8. **是否需要迁移007？** 需要。
9. **是否需要notification_deliveries？** 需要。
10. **是否需要notification_channel_bindings？** 需要。
11. **接收人如何绑定？** 企业微信通过官方成员 UserID 人工核验；iLink PoC
    只手工配置专用测试接收人 ID，禁止姓名猜测。
12. **Gateway如何认证？** 私网 mTLS + 独立 HMAC 签名、时间戳、nonce 和
    body hash。
13. **Gateway允许公网访问吗？** 不允许默认公网暴露。
14. **如何处理登录失效？** 停止领取、标记 `login_required`、人工重新登录，
    不自动绕过验证。
15. **如何处理发送结果未知？** `result_unknown` 隔离，禁止自动重发和降级。
16. **允许自动切企业微信吗？** V1不存在该切换，因为企业微信本身是正式主渠道，
    iLink不是fallback；未来多渠道只对明确未发送且单独批准的规则开放。
17. **如何避免重复发送？** 稳定 delivery 幂等键、Gateway 持久去重、第一条
    sent 取消其他、unknown 不重试。
18. **消息允许哪些客户信息？** 不允许客户名称或客户明细；只允许通知类型、
    数量、业务日期/时间、简短行动提示和需登录H5链接。
19. **H5链接如何处理权限？** 普通 HTTPS 站内路径，必须登录并实时授权，URL
    不携带 JWT 或签名授权。
20. **V1是否只发文本？** 是。
21. **是否完全禁止接收微信指令？** 是。PoC只观察测试会话与context token，
    不提供业务命令处理能力。
22. **PoC使用什么账号和数据？** 专用测试发送/接收账号和合成消息，不连接
    生产库或真实 outbox。
23. **哪些风险阻止生产？** 授权不明、context token 阻止主动发送、无法隔离
    凭证、无 delivery/binding、unknown 无法处理、需 Hook/绕过风控。
24. **是否有阻塞实现的未决项？** 没有产品设计未决项；企业微信主体、应用、
    UserID和专用PoC账号属于对应实施前的外部门禁。
25. **推荐后续顺序？** 冻结设计提交 → 隔离iLink Gateway → 3至7天功能PoC
    → 可选30天观察 → 核验企业微信外部门禁 → 迁移007与正式渠道基础设施
    → 企业微信单用户灰度；iLink接真实outbox需再次审批。
...
1. 提交本设计的独立冻结基线；
2. 另行批准并开发完全隔离的 iLink PoC Gateway；
3. 用固定合成消息完成 3 至 7 天功能 PoC；
4. 根据结果决定是否进入 30 天稳定性观察；
5. 单独核验企业微信主体、自建应用和成员 UserID 条件；
6. 设计并实现迁移 `007`、delivery 和 binding 存储层；
7. 重构通知 Worker 渠道路由并完成 Mock 回归；
8. 实现 Gateway mTLS/HMAC、防重放和 Fake Gateway；
9. 实现企业微信 Adapter、绑定工具与只读管理 API；
10. 完成消息隐私、H5权限、result_unknown和回滚测试；
11. 使用单个内部测试用户灰度企业微信；
12. iLink 只有再次获得生产审批后才能接入真实 outbox。

本文已冻结，但本轮到此停止，不自动进入实现、PoC、登录或发送阶段。
