...
> H5-only 后续决策说明：本文中微信小程序构建、旧小程序客户端等表述是 H5-only
> 决策前的历史设计事实；之后不再构建、发布或验收小程序。本文当时关于普通微信/
> 企业微信独立规划的表述已被 2026-08-01 用户决定覆盖：企业微信取消，所有真实
> 外部消息渠道暂停且不作为待实施规划；阶段三 outbox、规则、租约、重试、TTL、
> 审计和 Mock 验证继续保留。该变化不影响后端对旧手工请求的安全拒绝要求。
