...
- [Tencent/openclaw-weixin 官方仓库](https://github.com/Tencent/openclaw-weixin)：审计日公开仓库显示 `2.0.x` 为 Active，README 当前列出 OpenClaw `>=2026.3.22`；`1.0.x` 仅 legacy maintenance。此数值只是当天公开资料，绝不是本项目源码常量。
...
6. README 说明登录凭证会由官方运行时本地保存，但未给出可供本项目安全依赖的稳定、版本化 session 文件格式。因此 Gateway 不创建、读取或导入 `session.json`，不猜测凭证路径或字段，也不复制 token/context token 到自己的状态库。
7. [OpenClaw 官方 CLI 文档](https://docs.openclaw.ai/cli/channels) 公开 `channels status --probe --json` 和 `channels capabilities --channel … --json`。补丁只接受公开 `actions` 的 `send`/`message.send` 或明确 `send=true` 作为出站桥接证据；结构不可解析、未声明能力、版本不符或无法验证 CLI 回执契约均为 `ILINK_SEND_CONTRACT_UNVERIFIED` 并失败关闭。
...
| 运行环境 | 官方插件依赖 OpenClaw；本轮独立 Gateway 不安装、不嵌入该插件。运行时检查实际 CLI、插件 metadata 兼容范围和 capability；解析失败即 `ILINK_VERSION_UNSUPPORTED` 或 `ILINK_CAPABILITY_UNSUPPORTED`。 |
| 登录/二维码 | 仅人工交互式登录；本轮和离线验收均禁止。 |
| `to_user_id` | 来自官方会话/消息上下文；PoC 只允许仓库外固定测试接收人配置，绝不按姓名或通讯录猜测。 |
| `context_token` | 来自入站会话、有效范围和主动通知可用性尚待实况验证；只由官方运行时处理，Gateway 不读取、保存或输出它。 |
| 发送语义 | raw iLink JSON 的 `ret=0` 是明确成功；通用 OpenClaw CLI 不假定透传 raw ret。只有已验证的 `{ok:true, result:{messageId}}` 是“官方运行时已完成 provider ret 校验”的确认；如 envelope/result 有 `channel` 则必须匹配，provider `channelId` 仅作安全格式校验且不回显，Gateway 仅存哈希回执。其他包装形态失败关闭或待人工确认。 |
| 重启恢复 | 官方登录凭证由官方运行时保存；Gateway 不将其当作稳定公开文件契约。PoC 需实际验证重启、过期与重新扫码。 |
