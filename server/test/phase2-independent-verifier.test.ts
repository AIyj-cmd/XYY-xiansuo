...
test('006 可重复执行且不改变既有 001 至 005 checksum、完整性或禁止对象边界', () => {
  ...
  assert.deepEqual((database.prepare('SELECT version FROM schema_migrations ORDER BY version').all() as Array<{ version: string }>).map((row) => row.version), ['001', '002', '003', '004', '005', '006', '007', '008', '009']);
