...
export type PilotQueueInput = { recipientUserId: number; eventType: 'scheduled_follow_overdue' | 'daily_report'; businessDate: string; now: string; databasePath: string };
type Candidate = { id: number; event_type: string; event_source: string; operation_id: string; recipient_user_id: number; status: string; message_snapshot_json: string };
...
export function inspectPilotQueue(db: DatabaseSync, input: PilotQueueInput): Record<string, unknown> {
  const rows = db.prepare(`SELECT id,event_type,event_source,operation_id,recipient_user_id,status,message_snapshot_json
    FROM notification_logs WHERE ${CLAIMABLE_NOTIFICATION_WHERE} ORDER BY available_at,id`).all(input.now, input.now, input.now, input.now) as Candidate[];
