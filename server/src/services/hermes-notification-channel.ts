 ...
 * generation and opaque account reference. The external Hermes overlay resolves peer/token/cursor from its
...
export class HermesNotificationChannel {
  ...
  async send(recipient: { userId: number; generation: number; accountRef: string }, message: NotificationChannelMessage, idempotencyKey: string, signal: AbortSignal): Promise<ChannelResult> {
    if (!this.config.hermesEnabled || !this.config.hermesGatewayUrl || !this.config.hermesGatewaySecret || recipient.generation < 1 || !recipient.accountRef) return { status: 'permanent_failure', errorCode: 'HERMES_CHANNEL_DISABLED' };
    ...
    const body = JSON.stringify({ deliveryId: randomUUID(), idempotencyKey, recipientUserId: recipient.userId, recipientBindingGeneration: recipient.generation, recipientAccountRef: recipient.accountRef, title: message.title, body: message.body, detailUrl: DETAIL_URL, gatewaySendTimeoutMs: this.config.openclawGatewaySendTimeoutMs, workerTimeoutMs: this.config.openclawGatewayTimeoutMs });
