...
export type HermesBindingPublic = { status: 'unbound' | 'pending' | 'active' | 'disabled'; generation: number; expires_at: string | null };
export function publicHermesBinding(database: DatabaseSync, userId: number): HermesBindingPublic {
  const row = database.prepare('SELECT status,generation,binding_code_expires_at FROM hermes_bindings WHERE user_id=?').get(userId) as { status: HermesBindingPublic['status']; generation: number; binding_code_expires_at: string | null } | undefined;
  return row ? { status: row.status, generation: row.generation, expires_at: row.binding_code_expires_at } : { status: 'unbound', generation: 0, expires_at: null };
}
